        ...
        >
            <span class="relative z-0 inline-flex overflow-hidden rounded-lg border border-slate-200">
                @if ($paginator->onFirstPage())
                    <span aria-disabled="true" aria-hidden="true" class="flex items-center border-r border-slate-200 px-2 py-1.5 text-slate-300">
                ...
                @else
                    <button
                        ...
                        class="flex items-center border-r border-slate-200 px-2 py-1.5 text-slate-500 hover:bg-[var(--dt-primary-light, #eef2ff)] hover:text-slate-700 focus:outline-none focus:ring-1 focus:ring-[var(--dt-primary, #4f46e5)]"
                ...
                @foreach ($elements as $element)
                    @if (is_string($element))
                        <span aria-hidden="true" class="border-r border-slate-200 px-3 py-1.5 text-xs text-slate-400">{{ $element }}</span>
                    @endif
                        ...
                        @foreach ($element as $page => $url)
                            <span wire:key="dt-paginator-{{ $paginator->getPageName() }}-page-{{ $page }}" class="border-r border-slate-200 last:border-r-0">
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page" class="flex items-center bg-[var(--dt-primary, #4f46e5)] px-3 py-1.5 text-xs font-medium text-[var(--dt-primary-text, #ffffff)]">
                                        {{ $page }}
                                    </span>
                                @else
                                    <button
                                        ...
                                        x-on:click="{{ $scrollIntoViewJsSnippet }}"
                                        class="flex items-center px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-[var(--dt-primary-light, #eef2ff)] hover:text-slate-800 focus:outline-none focus:ring-1 focus:ring-[var(--dt-primary, #4f46e5)]"
                ...
                @if ($paginator->hasMorePages())
                    <button
                        ...
                        x-on:click="{{ $scrollIntoViewJsSnippet }}"
                        wire:loading.attr="disabled"
                        class="flex items-center px-2 py-1.5 text-slate-500 hover:bg-[var(--dt-primary-light, #eef2ff)] hover:text-slate-700 focus:outline-none focus:ring-1 focus:ring-[var(--dt-primary, #4f46e5)]"
                ...
                @else
                    <span aria-disabled="true" aria-hidden="true" class="flex items-center px-2 py-1.5 text-slate-300">
                        <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
                    </span>
                @endif
            </span>
